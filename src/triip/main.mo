    ...
    import WheelPrize "../triip_models/model/WheelPrize";
                ...
                if(current_kyc.status == ?"approved"){
                    ...
                    switch(activated_wheel){
                        case(? V){
                            ...
                            var result : [WheelPrize.WheelPrize] = [];
                            var itemIndex = 0;
                            // Picking the random item based on its weight
                            // The items with higher weight will be picked more often
                            while(itemIndex < cumulativeWeights.size()){
                                if(cumulativeWeights[itemIndex] >= randomPercentage){
                                    result:=Array.append<WheelPrize.WheelPrize>(result,[prizes[itemIndex]]);
                                };
                                itemIndex += 1;
                            };
                            
                            var spinResultIndex = state.spinresults.size() + 1;
                            if(result[0].prize_name == "Unlucky"){
                                let new_spin_result : Types.SpinResult = {
                                    id = Int.toText(spinResultIndex);
                                    user_id = Principal.toText(caller);
                                    prize_id = result[0].prize_id;
                                    prize_name = result[0].prize_name;
                                    state = "completed";
                                    remark : ?Text = Option.get(null,?"No need operation to process");
                                    created_at = Time.now();
                                    updated_at = Time.now();
                                };  
                                let updated_spin_result = state.spinresults.put(Int.toText(spinResultIndex),new_spin_result);
                                #ok((result[0].prize_id));
                            } else {
                                let new_spin_result : Types.SpinResult = {
                                    id = Int.toText(spinResultIndex);
                                    user_id = Principal.toText(caller);
    ...
    // Spin Result
    public shared query(msg) func listSpinResults() : async Result.Result<[(Text,Types.SpinResult)],Types.Error>{
        let uid = msg.caller;
